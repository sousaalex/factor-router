"""Router económico: teto de modelo + prompt budget (classifier)."""
from __future__ import annotations

import unittest
from unittest.mock import patch

from src.gateway.model_policy import cap_model_for_low_openrouter_credit
from src.router.classifier_prompt import build_classifier_prompt


class TestCapLowCredit(unittest.TestCase):
    def test_disabled_unchanged(self) -> None:
        self.assertEqual(
            cap_model_for_low_openrouter_credit("gemini/gemini-3.1-pro-preview", balance_low=False),
            "gemini/gemini-3.1-pro-preview",
        )

    def test_openrouter_frontier_to_kimi(self) -> None:
        with patch("src.router.router.get_model_info", return_value={"tier": "frontier"}):
            self.assertEqual(
                cap_model_for_low_openrouter_credit("openrouter/custom/frontier-model", balance_low=True),
                "openrouter/moonshotai/kimi-k2.5",
            )

    def test_native_gemini_not_capped_by_openrouter_budget(self) -> None:
        self.assertEqual(
            cap_model_for_low_openrouter_credit(
                "gemini/gemini-3.1-pro-preview",
                balance_low=True,
            ),
            "gemini/gemini-3.1-pro-preview",
        )

    def test_reasoning_unchanged(self) -> None:
        self.assertEqual(
            cap_model_for_low_openrouter_credit("openrouter/qwen/qwen3.5-397b-a17b", balance_low=True),
            "openrouter/qwen/qwen3.5-397b-a17b",
        )

    def test_simple_tier_unchanged_under_cap(self) -> None:
        self.assertEqual(
            cap_model_for_low_openrouter_credit("openrouter/qwen/qwen3.5-plus-02-15", balance_low=True),
            "openrouter/qwen/qwen3.5-plus-02-15",
        )

    def test_reasoning_plus_unchanged(self) -> None:
        self.assertEqual(
            cap_model_for_low_openrouter_credit("openrouter/moonshotai/kimi-k2.5", balance_low=True),
            "openrouter/moonshotai/kimi-k2.5",
        )


class TestClassifierBudgetPrompt(unittest.TestCase):
    def test_low_balance_appends_block(self) -> None:
        sys_low, _ = build_classifier_prompt(
            user_message="hi",
            models=[{"id": "openrouter/qwen/qwen3.5-397b-a17b", "tier": "reasoning", "pricing": {}, "description": "x"}],
            default_model="openrouter/qwen/qwen3.5-397b-a17b",
            openrouter_balance_low=True,
        )
        self.assertIn("OPENROUTER PREPAID BALANCE IS LOW", sys_low)

    def test_normal_no_block(self) -> None:
        sys_ok, _ = build_classifier_prompt(
            user_message="hi",
            models=[{"id": "openrouter/qwen/qwen3.5-397b-a17b", "tier": "reasoning", "pricing": {}, "description": "x"}],
            default_model="openrouter/qwen/qwen3.5-397b-a17b",
            openrouter_balance_low=False,
        )
        self.assertNotIn("OPENROUTER PREPAID BALANCE IS LOW", sys_ok)


if __name__ == "__main__":
    unittest.main()
